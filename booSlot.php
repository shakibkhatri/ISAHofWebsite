...
$success = array();
    ...
    if(!empty($guestName) || !empty($guestEmail) || !empty($campusId)) {
        ...
        if(mysqli_connect_error()) {
            ...
            if(strlen($campusId) > 8 || strlen($campusId) < 8) {
                array_push($errors, "Invalid Matrikel-Number");
            }
            $SELECT = "SELECT guestEmail FROM bookslot WHERE guestEmail = ? LIMIT 1";
            $INSERT = "INSERT INTO bookslot(guestName, guestEmail, campusId)
                              VALUES (?, ?, ?)";
                ...
                if($rnum==0) {
                    ...
                    $stmt->bind_param("ssi", $guestName, $guestEmail, $campusId);
                    $stmt->execute();
